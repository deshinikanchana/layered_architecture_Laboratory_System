...
public class ResetPasswordFormController {
    ...
    ResetPwBO bo = (ResetPwBO) BOFactory.getBoFactory().getBO(BOFactory.BOTypes.RESETPW);
    public void onActionSubmitBtn(ActionEvent event) throws SQLException, IOException, ClassNotFoundException {
        ...
        if(pw != null && cpw != null) {
            if(pw.equals(cpw)) {
                ud.setPassword(pw);
                boolean ans = bo.Update(ud);

                if(ans){
                    AnchorPane anchorPane = load(getClass().getResource("/lk.ijse.laboratory/assests/successAlert_form.fxml"));
